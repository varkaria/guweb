...
path_to_gulag = (
    _path_to_bancho_py
    if _path_to_bancho_py is not None
    else os.path.join(os.getcwd(), "../bancho.py") + "/"
)
...
create_api_key_if_not_exist = (
    _create_api_key_if_not_exist if _create_api_key_if_not_exist is not None else False
)

# only supporters can change their username through user edit page
name_change_only_for_supporter = (
    _name_change_only_for_supporter
    if _name_change_only_for_supporter is not None
    else True
)
...
discord_server = (
    ...
    if discord_invite_id is not None
    else None
)
