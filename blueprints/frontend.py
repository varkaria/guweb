...
def login_required(func):
    ...
    async def wrapper(*args, **kwargs):
        if not session:
            return await flash(
                "error", t("global.you-must-be-logged-in-to-access-this-page"), "login"
            )
...
async def settings_profile_post():
    ...
    if new_name == old_name and new_email == old_email:
        return await flash(
            "error", t("settings.profile.no-changes-have-been-made"), "settings/profile"
        )
    ...
    if new_name != old_name:
        ...
        if (
            not session["user_data"]["priv"] & Privileges.Supporter
            and glob.config.name_change_only_for_supporter
        ):
            return await flash(
                "error", t("username-changes-supporter-only"), "settings/profile"
            )

        if not regexes.username.match(new_name):
            return await flash(
                "error",
                t("settings.profile.new-username-is-invalid"),
                "settings/profile",
            )
        ...
        if "_" in new_name and " " in new_name:
            return await flash(
                "error",
                t("settings.profile.new-username-cannot-contains-both-dash-and-space"),
                "settings/profile",
            )
        ...
        if new_name in glob.config.disallowed_names:
            return await flash(
                "error",
                t("settings.profile.new-username-disallowed"),
                "settings/profile",
            )
        ...
        if await glob.db.fetch("SELECT 1 FROM users WHERE name = %s", [new_name]):
            return await flash(
                "error",
                t("settings.profile.new-username-already-taken-by-others"),
                "settings/profile",
            )
    ...
    if new_email != old_email:
        ...
        if not regexes.email.match(new_email):
            return await flash(
                "error", t("settings.profile.new-email-is-invalid"), "settings/profile"
            )
    ...
    return await flash(
        "success", t("settings.profile.succeed-please-login-again"), "login"
    )
...
async def settings_avatar_post():
    ...
    if avatar is None or not avatar.filename:
        return await flash(
            "error", t("settings.no-image-was-selected"), "settings/avatar"
        )
    ...
    return await flash(
        "success",
        t("settings.change-succeed", something=t("settings.avatar").lower()),
        "settings/avatar",
    )
...
async def settings_custom_post():
    ...
    if banner is None and background is None:
        return await flash_with_customizations(
            "error", t("settings.no-image-was-selected"), "settings/custom"
        )
    ...
    if banner is not None and banner.filename:
        ...
        if not file_extension in ALLOWED_EXTENSIONS:
            return await flash_with_customizations(
                "error",
                t(
                    "settings.bad-image-extension",
                    something=t("settings.banner").lower(),
                ),
                "settings/custom",
            )
    ...
    if background is not None and background.filename:
        ...
        if not file_extension in ALLOWED_EXTENSIONS:
            return await flash_with_customizations(
                ...
                t(
                    "settings.bad-image-extension",
                    something=t("settings.background").lower(),
                ),
                "settings/custom",
            )
    ...
    return await flash_with_customizations(
        "success",
        t("settings.change-succeed", something=t("settings.customization").lower()),
        "settings/custom",
    )
...
async def settings_aboutme_post():
    ...
    if "<iframe" in safe_content or "<script" in safe_content:
        return await render_template(
            "settings/aboutme.html",
            flash="Not allowed method",
            status="error",
    ...
    return await render_template(
        "settings/aboutme.html", userpage_content=userpage_content
    )
...
async def settings_password_post():
    ...
    if new_password != repeat_password:
        return await flash(
            "error",
            t("settings.password.new-passoword-mismatch-repeated-password"),
            "settings/password",
        )
    ...
    if old_password == new_password:
        return await flash(
            "error",
            t("settings.password.new-passoword-cannot-be-same-as-old-password"),
            "settings/password",
        )
    ...
    if not 8 < len(new_password) <= 32:
        return await flash(
            "error",
            t("settings.password.must-between-8-32-characters-in-length"),
            "settings/password",
        )
    ...
    if len(set(new_password)) <= 3:
        return await flash(
            "error",
            t("settings.password.must-have-more-than-3-unique-characters"),
            "settings/password",
        )

    if new_password.lower() in glob.config.disallowed_passwords:
        return await flash(
            "error", t("settings.password.too-simple"), "settings/password"
        )
    ...
    if pw_bcrypt in bcrypt_cache:
        if pw_md5 != bcrypt_cache[pw_bcrypt]:  # ~0.1ms
            ...
            return await flash(
                "error",
                t("settings.password.old-password-incorrect"),
                "settings/password",
            )
    else:  # ~200ms
    ...
    session.pop("user_data", None)
    return await flash(
        "success", t("settings.change-succeed", something=t("global.password")), "login"
    )
...
async def login_post():
    ...
    if api_key is None and glob.config.create_api_key_if_not_exist:
        ...
        await glob.db.execute(
            "UPDATE users SET api_key = %s WHERE id = %s", [api_key, user_info["id"]]
        )
...
async def register_post():
    ...
    if not regexes.username.match(username):
        return await flash(
            "error", t("register.username-syntax-is-invalid"), "register"
        )

    if "_" in username and " " in username:
        return await flash(
            "error",
            t("register.username-cannot-contains-both-dash-and-space"),
            "register",
        )
    ...
    if await glob.db.fetch("SELECT 1 FROM users WHERE name = %s", username):
        return await flash(
            "error", t("register.username-already-taken-by-others"), "register"
        )
    ...
    if not 8 <= len(passwd_txt) <= 32:
        return await flash(
            "error",
            t("register.password.must-between-8-32-characters-in-length"),
            "register",
        )
    ...
    if len(set(passwd_txt)) <= 3:
        return await flash(
            "error",
            t("register.password.must-have-more-than-3-unique-characters"),
            "register",
        )
